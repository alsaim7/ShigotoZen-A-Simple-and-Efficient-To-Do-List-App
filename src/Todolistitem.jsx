...
export default function Todolistitem({ l, deleteTask, checkTask }) {
    ...
    return (
        ...
        >
            <CardContent sx={{ display: 'flex', alignItems: 'center', padding: '12px 16px', '&:last-child': { pb: '12px' } }}>
                <Checkbox
                    checked={l.completed}
                    onChange={() => checkTask(l.id)}
                    sx={{ color: 'text.secondary', '&.Mui-checked': { color: 'primary.main' }, padding: '8px' }}
                />
                <Typography
                    sx={{
                        flexGrow: 1,
                ...
                {/* Drag handle with touchAction set only here */}
                <IconButton
                    {...attributes}
                    {...listeners}
                    sx={{
                        ...
                        padding: '12px',
                        touchAction: 'none', // ✅ enables touch dragging
                        '& svg': { fontSize: '1.5rem' },
                    }}
                >
                    <DragHandleIcon />
